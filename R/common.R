...
income_group_composition <- function(d, name_col, income_col, weight_col) {
  ...
  tot <- sum(d$w)
  d[order(income_group, -w), .(areas = .N, people = sum(w), share = sum(w) / tot, inc_lo = min(inc), inc_hi = max(inc),
                                largest = paste(utils::head(nm, k), collapse = "; ")), keyby = .(group = income_group)]
}
...
income_group_lumpiness <- function(d, state, name_col, weight_col) {
  ...
  w <- d[[weight_col]]; tot <- sum(w)
  gs <- tapply(w, d$income_group, sum) / tot
  top <- d[which.max(w)]; sh <- max(w) / tot
  if (sh < 1 / n)
    return(sprintf("%s: the largest area (%s) holds only %s of earners, so group shares differ from 1/%d just because whole areas are kept together (%s to %s).",
                   state, top[[name_col]], pct1(sh), n, pct1(min(gs)), pct1(max(gs))))
  sprintf("%s: %s alone holds %s of the state's earners, more than a 1/%d slice. It cannot be split, so its group (Q%d) holds %s of earners and Q%s is left with only %s.",
...
analysis_windows <- function(flow) {
  ...
  lab <- function(a, b) { l <- span_label(a, b); if (uniqueN(l) == 1) l[1] else paste(w$state, l, collapse = "; ") }
