...
title <- function(sheet, text, sub = NULL) {
  put(sheet, text, 1)
  bold(sheet, "A1", size = 14)
  if (!is.null(sub)) {
    put(sheet, sub, 2)
    wb$add_font(sheet = sheet, dims = "A2", italic = TRUE, name = "Arial", size = 10, color = wb_color(MUTED))
  }
}
...
inputs <- data.table(
  item = c(
    "NSW new regos, private buyers: value of a suppressed '<=5' cell — BEV", "NSW new regos, private buyers: value of a suppressed '<=5' cell — PHEV",
  ...
  value = c(
    kv("flow", "bev")$k, kv("flow", "phev")$k, kv("flow", "other")$k, kv("stock", "bev")$k, kv("stock", "phev")$k,
    kv("stock", "other")$k, CFG$period$recent_window_months, CFG$analysis$min_new_regs_scatter, CFG$analysis$min_recent_vehicles_vic
  ),
  how = c(
    kv("flow", "bev")$method, kv("flow", "phev")$method, kv("flow", "other")$method, kv("stock", "bev")$method,
...
nf(s, "B5:B10", DEC)
IN <- list(
  kf_bev = "Inputs!$B$5", kf_phev = "Inputs!$B$6", kf_other = "Inputs!$B$7", ks_bev = "Inputs!$B$8",
...
putv(s, names(derived), 16, 1)
putv(s, FML(unname(derived)), 16, 2)
putv(s, unname(derived), 16, 3)
nf(s, "B16:B25", MON)
D <- list(
  nsw_last = "Inputs!$B$16", qld_last = "Inputs!$B$17", nsw_start = "Inputs!$B$18", qld_start = "Inputs!$B$19",
...
put(s, sprintf(
  "Derived: first month %s was >= %d%% above its trailing 12-month mean (%.1f vs %.1f c/L). config.yaml fuel.*",
  onset$onset_series, round(100 * onset$threshold), onset$onset_price, onset$trailing_mean
), 28, 3)
wb$add_fill(sheet = s, dims = "B28", color = wb_color("FFFF00"))
wb$add_font(sheet = s, dims = "B28", name = "Arial", size = 10, color = wb_color("0000FF"))
crisis <- c(
  "Crisis window end (latest month with data in both states)" = sprintf("MIN(%s,%s)", D$nsw_last, D$qld_last),
...
putv(s, names(crisis), 29, 1)
putv(s, FML(unname(crisis)), 29, 2)
putv(s, unname(crisis), 29, 3)
nf(s, "B28:B31", MON)
nf(s, "B32", "0")
nf(s, "B33:B34", MON)
D <- c(D, list(
  cr_start = "Inputs!$B$28", cr_end = "Inputs!$B$29", py_start = "Inputs!$B$30", py_end = "Inputs!$B$31",
...
oth <- data.table(
  item = c(
    "NSW new regos, business/dealer/government: value of a suppressed '<=5' cell — BEV",
...
head(s, 1, c(
  "State", "LGA", "Month", "Income group", "BEV exact", "BEV suppressed cells", "PHEV exact", "PHEV suppressed cells",
...
x[, k := FML(sprintf("E%d+%s*F%d", r, kk(IN$kf_bev, IN$kfo_bev), r))]
x[, l := FML(sprintf("G%d+%s*H%d", r, kk(IN$kf_phev, IN$kfo_phev), r))]
x[, m := FML(sprintf("I%d+%s*J%d", r, kk(IN$kf_other, IN$kfo_other), r))]
x[, n := FML(sprintf("K%d+L%d+M%d", r, r, r))]
x[, o := flow$customer]
put(s, x, 2)
NF <- max(r)
nf(s, dims(2, 3, NF), MON)
nf(s, dims(2, 11, NF, 14), "#,##0.0")
widths(s, c(7, 26, 10, 9, rep(11, 10), 20))
wb$freeze_pane(sheet = s, first_row = TRUE)

s <- "Stock_NSW_LGA"
setorder(snsw, lga_name, month)
head(s, 1, c(
  "LGA", "Month", "Income group", "BEV exact", "BEV suppressed cells", "PHEV exact", "PHEV suppressed cells",
...
x[, j := FML(sprintf("D%d+%s*E%d", r, IN$ks_bev, r))]
x[, k := FML(sprintf("F%d+%s*G%d", r, IN$ks_phev, r))]
x[, l := FML(sprintf("H%d+%s*I%d", r, IN$ks_other, r))]
x[, m := FML(sprintf("J%d+K%d+L%d", r, r, r))]
put(s, x, 2)
nf(s, dims(2, 2, max(r)), MON)
nf(s, dims(2, 10, max(r), 13), NUM)
widths(s, c(26, 10, 9, rep(11, 10)))
wb$freeze_pane(sheet = s, first_row = TRUE)
...
nf(s, dims(2, 2, nrow(sqld) + 1), MON)
widths(s, c(26, 10, 9, 22))
wb$freeze_pane(sheet = s, first_row = TRUE)
...
nf(s, dims(2, 2, nrow(vic) + 1), MON)
nf(s, dims(2, 4, nrow(vic) + 1, 8), NUM)
widths(s, c(10, 12, 9, 11, 9, 11, 12, 11))
wb$freeze_pane(sheet = s, first_row = TRUE)

s <- "Fuel_Prices"
FSER <- c(CFG$fuel$retail_series, CFG$fuel$tgp_series)
FLAB <- c(
  NSW_ULP = "NSW retail ULP", NSW_Diesel = "NSW retail diesel", QLD_ULP = "QLD retail ULP", QLD_Diesel = "QLD retail diesel",
...
put(s, fx, 2)
nf(s, dims(2, 1, nrow(fx) + 1), MON)
nf(s, dims(2, 2, nrow(fx) + 1, 1 + length(FSER)), ONE)
...
head(s, 4, c(
  "State", "LGA code", "LGA", "Median income ($)", "Earners", "Income group", paste0("New regos (", WIN$recent, ")"),
...
x[, o := FML(sprintf(
  'IF(A%d="NSW",SUMIFS(Stock_NSW_LGA!$J:$J,Stock_NSW_LGA!$A:$A,C%d,Stock_NSW_LGA!$B:$B,%s),SUMIFS(Stock_QLD_LGA!$D:$D,Stock_QLD_LGA!$A:$A,C%d,Stock_QLD_LGA!$B:$B,%s))',
...
x[, t := FML(sprintf(
  "IFERROR(SUMIFS(Flow_LGA_Month!$K:$K,%s%s)/SUMIFS(Flow_LGA_Month!$N:$N,%s%s),\"\")",
...
put(s, x, R0)
nf(s, dims(R0, 4, LN), USD)
nf(s, dims(R0, 5, LN), NUM)
nf(s, dims(R0, 7, LN, 9), NUM)
nf(s, dims(R0, 10, LN), PCT)
nf(s, dims(R0, 11, LN, 12), NUM)
nf(s, dims(R0, 13, LN), PCT)
nf(s, dims(R0, 14, LN), ONE)
nf(s, dims(R0, 15, LN, 16), NUM)
nf(s, dims(R0, 17, LN, 18), ONE)
nf(s, dims(R0, 20, LN, 21), PCT)
nf(s, dims(R0, 22, LN), ONE)
...
for (st_ in c("NSW", "QLD")) {
  i <- which(ls_$state == st_ & ls_$elig)
  scat[[st_]] <- R0 - 1 + range(i)
}

# ====================================================== VIC postcode ========
s <- "VIC_Postcode"
title(
  s, "VIC postcodes — fleet snapshot (quarterly) against ATO median taxable income",
...
head(s, 4, c(
  "Postcode", "Median taxable income ($)", "Individuals", "Income group", "Vehicles (latest)", "BEV (latest)",
...
x[, o := fifelse(
  is.na(suburbs$suburbs[match(postcode, suburbs$postcode)]), paste("Postcode", postcode),
...
put(s, x, R0)
nf(s, dims(R0, 2, PN), USD)
nf(s, dims(R0, 3, PN), NUM)
nf(s, dims(R0, 5, PN, 6), NUM)
nf(s, dims(R0, 7, PN), ONE)
nf(s, dims(R0, 8, PN, 9), NUM)
nf(s, dims(R0, 10, PN), PCT)
nf(s, dims(R0, 11, PN, 12), NUM)
nf(s, dims(R0, 13, PN), ONE)
nf(s, dims(R0, 16, PN), NUM)
nf(s, dims(R0, 17, PN), ONE)
nf(s, dims(R0, 18, PN), NUM)
nf(s, dims(R0, 19, PN), ONE)
...
s <- "Flow_Group_Month"
title(
  s, "New registrations by LGA income group — monthly",
...
hdr <- c(
  "Month", unlist(lapply(blocks, function(b) paste(b[1], b[2], GROUP_LABEL))),
  unlist(lapply(c("NSW", "QLD"), function(st_) c(paste(st_, "BEV share", GROUP_LABEL), paste(st_, "BEV share all"))))
)
head(s, 4, hdr)
M0 <- 5
r <- rows_of(length(months), M0)
MN <- max(r)
putv(s, months, M0, 1)
nf(s, dims(M0, 1, MN), MON)
col <- 2
for (b in blocks) {
  for (g in GROUPS) {
    putv(s, FML(sprintf(
      'SUMIFS(Flow_LGA_Month!$%s:$%s,Flow_LGA_Month!$A:$A,"%s",Flow_LGA_Month!$D:$D,%d,Flow_LGA_Month!$C:$C,$A%d)',
      b[3], b[3], b[1], g, r
    )), M0, col)
    col <- col + 1
  }
...
for (bi in 0:1) {
  ...
  for (g in GROUPS) {
    putv(s, FML(sprintf('IF(%s%d>0,%s%d/%s%d,"")', L(new0 + g - 1), r, L(bev0 + g - 1), r, L(new0 + g - 1), r)), M0, col)
    col <- col + 1
  }
  putv(s, FML(sprintf(
    'IF(SUM(%s%d:%s%d)>0,SUM(%s%d:%s%d)/SUM(%s%d:%s%d),"")', L(new0), r, L(new0 + NG - 1), r,
    L(bev0), r, L(bev0 + NG - 1), r, L(new0), r, L(new0 + NG - 1), r
  )), M0, col)
  col <- col + 1
}
nf(s, dims(M0, SH$NSW, MN, col - 1), PCT)
widths(s, c(9, rep(9, length(hdr) - 1)))
wb$freeze_pane(sheet = s, first_active_row = M0, first_active_col = 2)

# ================================================ How income groups are built ==
s <- "Income_Groups"
title(
  s, sprintf("How the income groups (Q1–Q%d) are built", NG),
...
r0 <- max(mrow) + 2
put(s, "What the groups contain", r0)
bold(s, dims(r0), size = 12)
...
grp_block <- function(r0, label, crit, src, cols, people, comp, note) {
  ...
  bold(s, dims(r0))
  head(s, r0 + 1, c(
    "Income group", "Areas", people, sprintf("Share of %s", tolower(people)), "Lowest area median ($)",
  ...
  put(s, x, r1)
  bold(s, dims(allr, 1, allr, 7))
  nf(s, dims(r1, 2, allr, 3), NUM)
  nf(s, dims(r1, 4, allr), PCT)
  nf(s, dims(r1, 5, allr, 6), USD)
  wb$add_cell_style(sheet = s, dims = dims(r1, 7, allr, 7), wrap_text = TRUE, vertical = "top")
  put(s, note, allr + 1)
  wb$merge_cells(sheet = s, dims = dims(allr + 1, 1, allr + 1, 7))
...
for (st_ in c("NSW", "QLD")) {
  d <- lga[state == st_]
  r0 <- grp_block(
    r0, sprintf("%s — council areas (LGAs), median total income %s", st_, CFG$income$lga_year), sprintf('"%s"', st_),
...
r0 <- grp_block(
  r0, sprintf("VIC — postcodes, median taxable income %s", CFG$income$postcode_year), '">0"',
...
title(
  s, sprintf("Who is buying the new BEVs? — %s vs %s", WIN$recent, WIN$base),
  sprintf(
    "Recent window = the latest %d months of data (%s). Baseline = the first %d months of data (%s). Window length is on Inputs. How the income groups are built: see Income_Groups.",
    WIN$months, WIN$recent, WIN$months, WIN$base
  )
)
head(s, 4, c(
  "State", "Income group", "Lowest LGA median ($)", "Highest LGA median ($)", "Earners", "New regos", "BEV",
...
for (st_ in c("NSW", "QLD")) {
  ...
  put(s, x, r0)
  bold(s, dims(allr, 1, allr, 14))
  tr <- allr + 1
  put(s, c(st_, "Top ÷ bottom group"), tr, 1)
  put(s, FML(sprintf("H%d/H%d", r0 + NG - 1, r0)), tr, 8)
  put(s, FML(sprintf("K%d/K%d", r0 + NG - 1, r0)), tr, 11)
  put(s, FML(sprintf("N%d/N%d", r0 + NG - 1, r0)), tr, 14)
  bold(s, dims(tr, 1, tr, 14))
  nf(s, dims(r0, 3, allr, 4), USD)
  nf(s, dims(r0, 5, allr, 7), NUM)
  nf(s, dims(r0, 8, allr, 10), PCT)
  nf(s, dims(r0, 11, allr), ONE)
  nf(s, dims(r0, 12, allr, 13), NUM)
  nf(s, dims(r0, 14, allr), PCT)
  nf(s, dims(tr, 8, tr, 14), DEC)
  grow[[st_]] <- r0
  r0 <- tr + 2
}
r0 <- r0 + 1
put(s, "VIC — postcode income groups (ATO 2023-24). Flow proxy = BEV share of recent-model vehicles in the latest snapshot.", r0)
bold(s, dims(r0, 1))
r0 <- r0 + 1
head(s, r0, c(
  "State", "Income group", "Lowest postcode median ($)", "Highest postcode median ($)", "Individuals",
...
put(s, x, r0)
bold(s, dims(allr, 1, allr, 13))
tr <- allr + 1
put(s, c("VIC", "Top ÷ bottom group"), tr, 1)
put(s, FML(sprintf("H%d/H%d", r0 + NG - 1, r0)), tr, 8)
put(s, FML(sprintf("M%d/M%d", r0 + NG - 1, r0)), tr, 13)
bold(s, dims(tr, 1, tr, 13))
nf(s, dims(r0, 3, allr, 4), USD)
nf(s, dims(r0, 5, allr, 7), NUM)
nf(s, dims(r0, 8, allr, 10), PCT)
nf(s, dims(r0, 11, allr), ONE)
nf(s, dims(r0, 12, allr), NUM)
nf(s, dims(r0, 13, allr), ONE)
nf(s, dims(tr, 8, tr, 13), DEC)
grow$VIC <- r0
...
s <- "Stock_Group"
title(
  s, "Registered fleet (stock) by income group",
...
stock_table <- function(r0, st_, parts, dates, ratios) {
  hdr <- c(
    "Month", unlist(lapply(parts, function(p) paste(st_, p$lab, GROUP_LABEL))),
    unlist(lapply(ratios, function(q) c(paste(st_, q$lab, GROUP_LABEL), paste(st_, q$lab, "all"))))
  )
  head(s, r0, hdr)
  first <- r0 + 1
  r <- rows_of(length(dates), first)
  putv(s, dates, first, 1)
  nf(s, dims(first, 1, max(r)), MON)
  col <- 2
  for (p in parts) {
    for (g in GROUPS) {
  ...
  for (q in ratios) {
    n0 <- 2 + q$num * NG
    for (g in GROUPS) {
      den <- if (is.numeric(q$den)) {
        sprintf("%s%d", L(2 + q$den * NG + g - 1), r)
      } else {
        sprintf('SUMIFS(LGA_Summary!$E:$E,LGA_Summary!$A:$A,"%s",LGA_Summary!$F:$F,%d)', st_, g)
      }
      putv(s, FML(sprintf('IF(%s>0,%s%d/%s*%d,"")', den, L(n0 + g - 1), r, den, q$scale)), first, col)
      col <- col + 1
    }
    dall <- if (is.numeric(q$den)) {
      sprintf("SUM(%s%d:%s%d)", L(2 + q$den * NG), r, L(2 + q$den * NG + NG - 1), r)
    } else {
      sprintf('SUMIFS(LGA_Summary!$E:$E,LGA_Summary!$A:$A,"%s")', st_)
    }
    putv(s, FML(sprintf("SUM(%s%d:%s%d)/%s*%d", L(n0), r, L(n0 + NG - 1), r, dall, q$scale)), first, col)
    col <- col + 1
...
STK <- list()
STK$NSW <- stock_table(
  4, "NSW", list(list(lab = "light vehicles", sheet = "Stock_NSW_LGA", col = "M"), list(lab = "BEV", sheet = "Stock_NSW_LGA", col = "J")),
  sort(unique(snsw$mdate)), list(
    list(lab = "BEV per 1,000 light vehicles", num = 1, den = 0, scale = 1000),
    list(lab = "BEV per 1,000 earners", num = 1, den = "earners", scale = 1000)
  )
)
STK$QLD <- stock_table(
  STK$NSW[2] + 3, "QLD", list(list(lab = "BEV seen", sheet = "Stock_QLD_LGA", col = "D")),
  sort(unique(sqld_q$mdate)), list(list(lab = "BEV per 1,000 earners", num = 0, den = "earners", scale = 1000))
)
STK$VIC <- stock_table(
  STK$QLD[2] + 3, "VIC", list(
    list(lab = "vehicles", sheet = "VIC_Postcode_Qtr", col = "D"),
  ...
  sort(unique(vic$qdate)), list(
    list(lab = "BEV per 1,000 vehicles", num = 1, den = 0, scale = 1000),
...
s <- "Fuel_Crisis"
title(
  s, "The 2026 fuel crisis — did the price shock change who buys BEVs?",
...
head(s, 4, c(
  "Month", "NSW retail ULP (c/L)", "NSW retail diesel (c/L)", "QLD retail ULP (c/L)", "QLD retail diesel (c/L)",
  "Terminal gate petrol (c/L)", "Crisis month? (1 = yes)", "NSW BEV share — all areas", "QLD BEV share — all areas"
))
X0 <- 5
r <- rows_of(length(months), X0)
XN <- max(r)
fr <- rows_of(length(months), M0)
...
nf(s, dims(X0, 1, XN), MON)
nf(s, dims(X0, 2, XN, 6), ONE)
nf(s, dims(X0, 8, XN, 9), PCT)
link(s, dims(X0, 8, XN, 9))
r0 <- XN + 3
put(s, "New registrations by income group — crisis months vs same months a year earlier", r0)
bold(s, dims(r0, 1))
r0 <- r0 + 1
head(s, r0, c(
  "State", "Income group", "New regos (crisis)", "BEV (crisis)", "BEV share (crisis)", "New regos (year earlier)",
...
for (st_ in c("NSW", "QLD")) {
  ...
  w <- function(colf, a, b) {
    sprintf(
      'SUMIFS(Flow_LGA_Month!$%s:$%s,Flow_LGA_Month!$A:$A,"%s"%s,Flow_LGA_Month!$C:$C,">="&%s,Flow_LGA_Month!$C:$C,"<="&%s)',
      colf, colf, st_, gc, a, b
    )
  }
  x <- data.table(
    st = st_, g = c(GROUP_LABEL, "All"), c = FML(w("N", D$cr_start, D$cr_end)), d = FML(w("K", D$cr_start, D$cr_end)),
  ...
  put(s, FML(sprintf("I%d-I%d", r0 + NG - 1, r0)), tr, 9)
  bold(s, dims(tr, 1, tr, 14))
  nf(s, dims(r0, 3, allr, 4), NUM)
  nf(s, dims(r0, 5, allr), PCT)
  nf(s, dims(r0, 6, allr, 7), NUM)
  nf(s, dims(r0, 8, allr), PCT)
  nf(s, dims(r0, 9, allr), ONE)
  nf(s, dims(r0, 10, allr), DEC)
  nf(s, dims(r0, 11, allr), NUM)
  nf(s, dims(r0, 12, allr, 14), PCT)
  nf(s, dims(tr, 5, tr, 10), DEC)
  nf(s, dims(tr, 9), ONE)
  xgrow[[st_]] <- r0
  r0 <- tr + 2
}
...
r0 <- r0 + 1
head(s, r0, c(
  "State", "Income group", "BEV fleet latest quarter", "BEV fleet quarter before", "Added (latest quarter)",
...
x <- data.table(
  st = "VIC", g = c(GROUP_LABEL, "All"), c = FML(vq("E", 0)), d = FML(vq("E", -3)), e = FML(sprintf("C%d-D%d", r, r)),
...
put(s, FML(sprintf("I%d/I%d", r0 + NG - 1, r0)), tr, 9)
put(s, FML(sprintf("K%d/K%d", r0 + NG - 1, r0)), tr, 11)
put(s, FML(sprintf("L%d/L%d", r0 + NG - 1, r0)), tr, 12)
bold(s, dims(tr, 1, tr, 13))
nf(s, dims(r0, 3, allr, 8), NUM)
nf(s, dims(r0, 9, allr), DEC)
nf(s, dims(r0, 10, allr), NUM)
nf(s, dims(r0, 11, allr, 12), ONE)
nf(s, dims(r0, 13, allr), PCT)
nf(s, dims(tr, 9, tr, 12), DEC)
xgrow$VIC <- r0
widths(s, c(9, 14, rep(12, 17)))
wb$freeze_pane(sheet = s, first_active_row = 5, first_active_col = 3)

# ========================================================= Customer types ===
s <- "Customer_Types"
title(
  s, "New registrations by customer type — NSW and QLD",
  paste0(
    "Every table in this workbook counts all customer types. NSW splits private, business, dealer (demonstrator) and government buyers; QLD only individuals vs organisations. ",
...
put(s, paste0("A. BEV take-up by customer type — ", WIN$recent, " and the fuel crisis"), 4)
bold(s, "A4")
head(s, 5, c(
  "State", "Customer type", "New regos", "BEV", "BEV share", "Share of state's new regos", "Share of state's BEVs",
...
for (st_ in c("NSW", "QLD")) {
  ty <- c(CT[[st_]], NA)
  r <- rows_of(length(ty), r0)
  allr <- max(r)
  x <- data.table(
    a = st_, b = ifelse(is.na(ty), "All customers", ty),
    ...
    h = FML(vapply(ty, function(t) {
      sprintf(
        "IFERROR(%s/%s,\"\")", fsum("K", st_, paste0(ctc(t), between(D$cr_start, D$cr_end))),
        fsum("N", st_, paste0(ctc(t), between(D$cr_start, D$cr_end)))
      )
    }, "")),
    i = FML(vapply(ty, function(t) {
      sprintf(
        "IFERROR(%s/%s,\"\")", fsum("K", st_, paste0(ctc(t), between(D$py_start, D$py_end))),
  ...
  put(s, x, r0)
  bold(s, dims(allr, 1, allr, 10))
  nf(s, dims(r0, 3, allr, 4), NUM)
  nf(s, dims(r0, 5, allr, 9), PCT)
  nf(s, dims(r0, 10, allr), ONE)
  ctrow[[st_]] <- c(r0, allr - 1)
  r0 <- allr + 2
}
put(s, paste0("B. BEV share of new regos by income group and customer type — ", WIN$recent), r0)
bold(s, dims(r0, 1))
r0 <- r0 + 1
head(s, r0, c("State", "Customer type", GROUP_LABEL, "Top ÷ bottom group"))
r0 <- r0 + 1
ctgrp <- list()
for (st_ in c("NSW", "QLD")) {
  ty <- c(NA, CT[[st_]])
  ...
  for (g in GROUPS) {
    putv(s, FML(vapply(ty, function(t) {
      ex <- paste0(ctc(t), sprintf(",Flow_LGA_Month!$D:$D,%d", g), since(st_))
      sprintf("IFERROR(%s/%s,\"\")", fsum("K", st_, ex), fsum("N", st_, ex))
    }, "")), r0, 2 + g)
  }
  putv(s, FML(sprintf('IFERROR(%s%d/C%d,"")', L(2 + NG), r, r)), r0, 3 + NG)
  nf(s, dims(r0, 3, max(r), 2 + NG), PCT)
  nf(s, dims(r0, 3 + NG, max(r)), DEC)
  ctgrp[[st_]] <- r0
  r0 <- max(r) + 2
}
put(s, paste0("C. Share of each income group's new BEVs bought by customers other than private buyers — ", WIN$recent), r0)
bold(s, dims(r0, 1))
r0 <- r0 + 1
head(s, r0, c("State", "", GROUP_LABEL, "All groups"))
r0 <- r0 + 1
for (st_ in c("NSW", "QLD")) {
  ...
  for (g in c(GROUPS, NA)) {
    ...
    put(
      s, FML(sprintf("IFERROR(1-%s/%s,\"\")", fsum("K", st_, paste0(ctc(PRIVATE), gx, since(st_))), fsum("K", st_, paste0(gx, since(st_))))),
      r0, if (is.na(g)) 3 + NG else 2 + g
    )
  }
  nf(s, dims(r0, 3, r0, 3 + NG), PCT)
  r0 <- r0 + 1
}
...
s <- "Raw_Numbers"
title(
  s, "Raw numbers — vehicle counts, not shares",
...
RN0 <- 5
r <- rows_of(length(months), RN0)
RNN <- max(r)
putv(s, FML(sprintf("Flow_Group_Month!A%d", fr)), RN0, 1)
nf(s, dims(RN0, 1, RNN), MON)
for (bi in 0:1) {
  new0 <- 2 + bi * 2 * NG
  bev0 <- new0 + NG
  c0 <- 2 + bi * 3
...
GC <- 10
head(s, 4, c(
  "Income group", paste0("NSW new BEVs (", WIN$recent, ")"), paste0("QLD new BEVs (", WIN$recent, ")"), "VIC recent-model BEVs in fleet",
  "NSW BEV fleet (latest)", "QLD BEVs seen since 2022 (latest)", "VIC BEV fleet (latest)", "NSW new BEVs — year before crisis",
  "NSW new BEVs — crisis months", "QLD new BEVs — year before crisis", "QLD new BEVs — crisis months"
), col = GC)
gi <- GROUPS - 1
x <- data.table(
  g = GROUP_LABEL,
...
nf(s, dims(RN0, GC + 1, RG_ALL, GC + 10), NUM)
bold(s, dims(RG_ALL, GC, RG_ALL, GC + 10))
link(s, dims(RN0, GC + 1, RN0 + NG - 1, GC + 10))
...
s <- "Top10"
title(
  s, sprintf("Top and bottom %d areas", TOPN),
...
specs <- list(
  ...
  list(
    "VIC suburbs (postcodes) — most BEVs added in the crisis quarter (count)", "VIC", NA, "P", -1,
...
for (k in seq_along(specs)) {
  ...
  put("Rank_Keys", sp[[1]], 3, k + 1)
  putv("Rank_Keys", FML(key), R0, k + 1)
  show <- c(if (src == "LGA") lga_show else vic_show, sp[[6]])
  col0 <- if (k %% 2 == 1) 1 else 10
  put(s, sp[[1]], row, col0)
  bold(s, dims(row, col0))
  ...
  top_tables[[k]] <- list(
    title = sp[[1]], r1 = row + 2, r2 = row + 1 + TOPN, c1 = col0 + 1, c2 = col0 + length(show), src = src,
...
s <- "Suppression"
title(
  s, "NSW small-cell suppression — how the '<=5' cells are valued",
...
put(s, supp[, .(table, customers, fuel_group, k, method)], 5)
SR <- 4 + nrow(supp)
nf(s, dims(5, 4, SR), DEC)
notes <- c(
...
wb$set_row_heights(sheet = s, rows = NR, heights = 62)
...
sections <- list(
  list("1. Raw numbers — how many BEVs", list(
    ...
    chart_bar(
      paste0("New BEVs registered in ", WIN$recent, ", by income group (count)"), "Raw_Numbers", rng(GC, GRP[1], GRP[2]),
      list(
        list(name = "NSW", ref = rng(GC + 1, GRP[1], GRP[2]), colour = STATE_COL[["NSW"]]),
    ...
    chart_bar(
      "BEVs in the registered fleet, latest, by income group (count)", "Raw_Numbers", rng(GC, GRP[1], GRP[2]),
      list(
        list(name = "NSW", ref = rng(GC + 4, GRP[1], GRP[2]), colour = STATE_COL[["NSW"]]),
  ...
  list("2. Income — BEV share and fleet rates", list(
    ...
    chart_bar(paste0("BEV share of new vehicles, ", WIN$recent, ", by income group"), "Flow_Group_Summary", rng(2, grow$NSW, grow$NSW + NG - 1),
      list(
        list(name = "NSW (new regos)", ref = rng(8, grow$NSW, grow$NSW + NG - 1), colour = STATE_COL[["NSW"]]),
        list(name = "QLD (new regos)", ref = rng(8, grow$QLD, grow$QLD + NG - 1), colour = STATE_COL[["QLD"]]),
        list(name = "VIC (recent-model vehicles in fleet)", ref = rng(8, grow$VIC, grow$VIC + NG - 1), colour = STATE_COL[["VIC"]])
      ),
      y_fmt = PCT
    ),
    chart_line("NSW fleet — BEVs per 1,000 light vehicles, by LGA income group", "Stock_Group", rng(1, STK$NSW[1], nl),
      grp_series(2 + 2 * NG, STK$NSW[1], nl),
      y_fmt = "0", y_title = "per 1,000"
    ),
    chart_line("VIC fleet — BEVs per 1,000 vehicles, by postcode income group", "Stock_Group", rng(1, STK$VIC[1], vl_),
      grp_series(2 + 4 * NG, STK$VIC[1], vl_),
      y_fmt = "0", y_title = "per 1,000"
    ),
    chart_scatter(
      paste0("NSW LGAs — median income vs BEV share of new regos (", WIN$recent, ")"), "LGA_Summary",
    ...
    chart_scatter("VIC postcodes — median taxable income vs BEVs per 1,000 vehicles", "VIC_Postcode",
      ...
      "Postcode median taxable income ($, 2023-24)", "BEV per 1,000",
      y_fmt = "0"
    )
  )),
  list("3. The 2026 fuel crisis", list(
    chart_line("Pump prices — the 2026 fuel crisis (retail, c/L)", "Fuel_Crisis", rng(1, X0, XN),
      list(
        list(name = "NSW ULP", ref = rng(2, X0, XN), colour = STATE_COL[["NSW"]]),
        list(name = "NSW diesel", ref = rng(3, X0, XN), colour = "9AA5B1", dash = TRUE),
        list(name = "QLD ULP", ref = rng(4, X0, XN), colour = STATE_COL[["QLD"]])
      ),
      y_fmt = "0", y_title = "c/L"
    ),
    chart_bar(
      "New BEVs by income group — crisis months vs same months a year earlier (count)", "Raw_Numbers", rng(GC, GRP[1], GRP[2]),
      list(
        list(name = "NSW year earlier", ref = rng(GC + 7, GRP[1], GRP[2]), colour = "B3CCE8"),
    ...
    chart_bar("NSW — BEV share by income group: crisis months vs same months a year earlier", "Fuel_Crisis", rng(2, xgrow$NSW, xgrow$NSW + NG - 1),
      list(
        list(name = "Same months a year earlier", ref = rng(8, xgrow$NSW, xgrow$NSW + NG - 1), colour = OTHER),
        list(name = "Fuel-crisis months", ref = rng(5, xgrow$NSW, xgrow$NSW + NG - 1), colour = STATE_COL[["NSW"]])
      ),
      y_fmt = PCT
    ),
    chart_bar("QLD — BEV share by income group: crisis months vs same months a year earlier", "Fuel_Crisis", rng(2, xgrow$QLD, xgrow$QLD + NG - 1),
      list(
        list(name = "Same months a year earlier", ref = rng(8, xgrow$QLD, xgrow$QLD + NG - 1), colour = OTHER),
        list(name = "Fuel-crisis months", ref = rng(5, xgrow$QLD, xgrow$QLD + NG - 1), colour = STATE_COL[["QLD"]])
      ),
      y_fmt = PCT
    )
  )),
  list("4. Customer types (tables on the Customer_Types sheet)", list(
    chart_bar(paste0("NSW — BEV share of new regos by customer type, ", WIN$recent), "Customer_Types", rng(2, ctrow$NSW[1], ctrow$NSW[2]),
      list(list(name = "BEV share", ref = rng(5, ctrow$NSW[1], ctrow$NSW[2]), colour = STATE_COL[["NSW"]])),
      y_fmt = PCT, legend = FALSE
    ),
    chart_bar("NSW — share of new BEVs by customer type", "Customer_Types", rng(2, ctrow$NSW[1], ctrow$NSW[2]),
      list(
        list(name = "Share of state's BEVs", ref = rng(7, ctrow$NSW[1], ctrow$NSW[2]), colour = STATE_COL[["NSW"]]),
        list(name = "Share of state's new regos", ref = rng(6, ctrow$NSW[1], ctrow$NSW[2]), colour = OTHER)
      ),
      y_fmt = PCT
    ),
    chart_bar(paste0("NSW — BEV share by income group: all customers vs private buyers, ", WIN$recent), "Customer_Types",
      ...
      list(
        list(name = "All customers", ref = sprintf("$C$%d:$%s$%d", ctgrp$NSW, L(2 + NG), ctgrp$NSW), colour = STATE_COL[["NSW"]]),
        list(name = "Private buyers", ref = sprintf("$C$%d:$%s$%d", ctgrp$NSW + 1, L(2 + NG), ctgrp$NSW + 1), colour = OTHER)
      ),
      y_fmt = PCT
    ),
    chart_bar(paste0("QLD — BEV share by income group: all customers vs individuals, ", WIN$recent), "Customer_Types",
      sprintf("$C$%d:$%s$%d", ctgrp$NSW - 1, L(2 + NG), ctgrp$NSW - 1),
      list(
        list(name = "All customers", ref = sprintf("$C$%d:$%s$%d", ctgrp$QLD, L(2 + NG), ctgrp$QLD), colour = STATE_COL[["QLD"]]),
        list(name = "Private buyers", ref = sprintf("$C$%d:$%s$%d", ctgrp$QLD + 1, L(2 + NG), ctgrp$QLD + 1), colour = OTHER)
      ),
      y_fmt = PCT
    )
  )),
...
for (sec in sections) {
  put(s, sec[[1]], row)
  bold(s, dims(row), col = PRIMARY, size = 13)
  row <- row + 1
  chs <- sec[[2]]
...
s <- "Summary"
title(
  s, "EV take-up by regional income — key metrics",
...
rows <- list(
  ...
  list(
    "New BEVs registered", c(sprintf("%s!%s%d", rn, L(GC + 1), RG_ALL), sprintf("%s!%s%d", rn, L(GC + 2), RG_ALL), NA),
    "NSW and QLD new registrations", NUM
  ),
  list(
    "BEV share of new cars", c(sprintf("%s!H%d", fg, gN + NG), sprintf("%s!H%d", fg, gQ + NG), sprintf("%s!H%d", fg, gV + NG)),
    "VIC: share of recent-model vehicles in the fleet", PCT
  ),
  list(
    "BEV share, poorest → richest fifth of areas",
    c(
      arrow(sprintf("%s!H%d", fg, gN), sprintf("%s!H%d", fg, gN + top), "0.0%"), arrow(sprintf("%s!H%d", fg, gQ), sprintf("%s!H%d", fg, gQ + top), "0.0%"),
      arrow(sprintf("%s!H%d", fg, gV), sprintf("%s!H%d", fg, gV + top), "0.0%")
    ), NA, NA
  ),
  list(
    sprintf("Richest ÷ poorest: %s → %s", WIN$base, WIN$recent), c(
      xarrow(sprintf("%s!N%d", fg, gN + NG + 1), sprintf("%s!H%d", fg, gN + NG + 1)),
  ...
  list(
    "BEVs in the fleet", c(sprintf("%s!%s%d", rn, L(GC + 4), RG_ALL), sprintf("%s!%s%d", rn, L(GC + 5), RG_ALL), sprintf("%s!%s%d", rn, L(GC + 6), RG_ALL)),
    "QLD: BEVs seen in registration records since 2022 (no public fleet data by region)", NUM
  ),
  list(
    "BEVs per 1,000 vehicles, poorest → richest fifth",
    c(
      arrow(sprintf("Stock_Group!%s%d", L(2 + 2 * NG), nl), sprintf("Stock_Group!%s%d", L(2 + 2 * NG + NG - 1), nl), "0"), NA,
      arrow(sprintf("Stock_Group!%s%d", L(2 + 4 * NG), vl_), sprintf("Stock_Group!%s%d", L(2 + 4 * NG + NG - 1), vl_), "0")
    ), NA, NA
  ),
  list("2026 fuel crisis — crisis months vs same months a year earlier", NULL),
  list(
    "New BEVs registered", c(
      arrow(sprintf("%s!G%d", fc, xN + NG), sprintf("%s!D%d", fc, xN + NG), "#,##0"),
  ...
  list("BEV share of new cars", c(
    arrow(sprintf("%s!H%d", fc, xN + NG), sprintf("%s!E%d", fc, xN + NG), "0.0%"),
    arrow(sprintf("%s!H%d", fc, xQ + NG), sprintf("%s!E%d", fc, xQ + NG), "0.0%"), NA
  ), NA, NA),
  list(
    "Growth in BEV share, poorest vs richest fifth", c(
      sprintf('%s&" vs "&%s', xs(sprintf("%s!J%d", fc, xN)), xs(sprintf("%s!J%d", fc, xN + top))),
...
for (x in rows) {
  put(s, x[[1]], i, 1)
  if (is.null(x[[2]])) {
    bold(s, dims(i, 1), col = PRIMARY)
    wb$add_fill(sheet = s, dims = dims(i, 1, i, 5), color = wb_color(BAND))
  } else {
    for (j in 1:3) {
      if (!is.na(x[[2]][j])) {
        put(s, FML(x[[2]][j]), i, j + 1)
        if (!is.na(x[[4]])) nf(s, dims(i, j + 1), x[[4]])
      } else {
        put(s, "—", i, j + 1)
      }
    }
    ...
    link(s, dims(i, 2, i, 4))
    wb$add_cell_style(sheet = s, dims = dims(i, 2, i, 4), horizontal = "right")
  }
  i <- i + 1
}
i <- i + 1
put(s, "How to read this", i)
bold(s, dims(i))
guide <- c(
...
put(s, "Individual files downloaded", r0)
bold(s, dims(r0))
...
s <- "Data_Adjustments"
title(
  s, "Summary of adjustments to the data",
...
extra <- data.table(
  dataset = c("NSW new registrations", "NSW fleet", "All", "All", "Fuel crisis"),
  adjustment = c(
    "Suppressed-cell value (flow)", "Suppressed-cell value (stock)", "Income groups within each state",
